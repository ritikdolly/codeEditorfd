...
export function CreateTest() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      // Helper to format date to yyyy-MM-ddTHH:mm:ss (no Z, no millis) as expected by LocalDateTime
      ...
      const payload = {
        ...
        startTime: formatToLocalISO(form.startTime),
        endTime: formatToLocalISO(form.endTime),
  ...
  const difficultyColor = {
    EASY: "text-accent bg-accent/5 border-accent/20",
        ...
        <form onSubmit={handleSubmit} className="space-y-12">
             ...
             <div className="p-8 space-y-10">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 pt-6 border-t border-white/5">
                  ...
                  <div className="space-y-3">
                    ...
                    <input
                      className="w-full bg-[#1a1a1a] border border-white/5 rounded-xl text-white px-5 py-3.5 focus:outline-none focus:border-accent font-bold shadow-xl appearance-none"
                      type="datetime-local"
                  ...
                  <div className="space-y-3">
                    <label className="text-[11px] font-bold text-gray-500 uppercase tracking-widest ml-1 opacity-50">End Time</label>
                    <input
          ...
          <div className="bg-[#111111] border border-white/10 rounded-2xl shadow-2xl overflow-hidden group">
             <div className="px-8 py-5 border-b border-white/5 bg-white/2 flex items-center justify-between">
                ...
                <span className="text-[10px] font-bold text-accent bg-white/5 px-4 py-2 rounded-xl border border-accent/20 uppercase tracking-widest">
             ...
             <div className="p-8">
                {questions.length === 0 ? (
                  <div className="p-16 bg-white/1 border border-white/5 border-dashed rounded-4xl flex flex-col items-center justify-center text-center shadow-2xl">
                    ...
                    <button
                      ...
                      className="mt-8 text-accent font-bold text-[11px] uppercase tracking-widest hover:text-white transition-colors"
                    >
                      Authorize New Question
                    </button>
                  </div>
                ) : (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-h-135 overflow-y-auto custom-scrollbar pr-3">
                    {questions.map((q) => {
                      ...
                      return (
                        <div
                          ...
                          className={`p-6 rounded-2xl border transition-all flex items-center gap-5 cursor-pointer group/item ${
                            selected
                              ? "border-accent/50 bg-accent/5 shadow-accent/5"
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            <p className={`font-bold text-[16px] tracking-tight truncate mb-1 ${selected ? "text-accent" : "text-white"}`}>
                              {q.title}
                            </p>
                            <div className={`flex items-center gap-3 font-bold text-[10px] uppercase tracking-widest ${selected ? "text-accent/60" : "text-gray-600"}`}>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-8 pt-10 border-t border-white/5">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full sm:w-auto bg-accent hover:bg-accent-dark text-black font-bold py-4 px-12 rounded-xl transition-all flex items-center justify-center gap-3 shadow-2xl active:scale-95 disabled:opacity-50 uppercase tracking-widest text-[12px] shadow-accent/10"
            >
